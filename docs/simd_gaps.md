...
| --- | --- | --- | --- |
| widen | no direct vector widen from narrow lanes to wider lanes；request recorded in `docs/uya_compiler_requests.md` | SAD、sub-pixel filter、loop filter 中的 `u8 -> i16/i32` 中间值需要额外步骤 | scalar tile helper 或拆成多个 scalar lanes；x86 SAD 16x16 已有可选 `@asm` microkernel |
| narrow / truncate / convert | no direct vector narrow or lane conversion builtin；request recorded in `docs/uya_compiler_requests.md` | residual add/clamp、inverse transform 后写回 `u8` 时缺少 `i16/i32 -> u8` vector path | scalar clamp and store；未来可用 `@vector.select` 组合局部替代 |
...
| unsigned saturating add/sub | signed vector `+|` / `-|` works, unsigned vector saturating is rejected；request recorded in `docs/uya_compiler_requests.md` | VP8 pixel math needs unsigned clamp semantics and narrow-to-u8 saturation | signed intermediate plus scalar clamp, or explicit compare/select when practical |
