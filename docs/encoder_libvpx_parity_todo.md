...
本轮范围：只生成本文档和 `docs/encoder_libvpx_parity_design.md`，不实现代码、不改 CLI、不新增脚本、不改 `Makefile`、不下载工具、不下载真实样本、不运行完整测试。后续实现必须另起目标。
...
- [x] 创建 `docs/encoder_libvpx_parity_todo.md`。
  - 验收：文件存在，未来实现任务均保持 `[ ]`。
- [x] 执行文档级 whitespace 检查。
  - 验收命令：`git diff --check`。
- [x] 确认本轮范围只包含两份专题文档。
  - 验收命令：`git status --short` 中本轮新增/修改项仅为 `docs/encoder_libvpx_parity_design.md` 和 `docs/encoder_libvpx_parity_todo.md`。
...
- [ ] 为 `encode` 增加 `--frames N` 参数解析。
  - 验收：`build/vp8uya encode input.i420 --width 16 --height 16 --frames 1 --out out.ivf` 与不传 `--frames` 行为一致。
- [ ] 拒绝 `--frames 0`。
  - 验收命令：`build/vp8uya encode input.i420 --width 16 --height 16 --frames 0 --out out.ivf` 返回参数错误。
- [ ] 拒绝非数字 `--frames`。
  - 验收：`--frames abc` 返回参数错误。
- [ ] 为 `encode` 增加 `--fps NUM/DEN` 参数解析。
  - 验收：`--fps 30/1` 写入 IVF timebase。
- [ ] 拒绝非法 `--fps`。
  - 验收：`--fps 0/1`、`--fps 30/0`、`--fps abc` 均返回参数错误。
- [ ] 实现多帧 I420 输入大小计算。
...
- [ ] 写出多帧 IVF header。
  - 验收：`build/vp8uya info out.ivf` 显示 `ivf.frame_count=N`。
- [ ] 写出逐帧 IVF timestamp。
...
- [ ] 验证本项目 decoder 可解多帧输出。
  - 验收：`build/vp8uya decode out.ivf --yuv out.i420` 输出大小为 `384 * 3` 字节。
- [ ] 验证 `vpxdec` 可解多帧输出。
  - 验收：`vpxdec --rawvideo -o out.vpxdec.i420 out.ivf` 输出大小为 `384 * 3` 字节。
- [ ] 验证 deterministic。
  - 验收：相同输入编码两次，`md5sum first.ivf second.ivf` 相同。
- [ ] 增加非 16 对齐多帧测试。
...
- [ ] 支持传入 `build/vp8uya` 路径。
  - 验收：二进制不存在时返回非 0 并打印路径。
- [ ] 支持 `--group` 样本过滤。
  - 验收：`--group qcif` 只运行 QCIF 样本。
- [ ] 支持 `--frames` 覆盖。
...
- [ ] 调用 `vp8uya encode --frames N --fps NUM/DEN`。
  - 验收：生成 `<sample>.vp8uya.ivf`。
- [ ] 调用 `vpxenc --best`。
  - 验收：生成 `<sample>.libvpx.ivf`。
- [ ] 调用 `vpxdec` 解码 UYA 输出。
  - 验收：生成 `<sample>.vp8uya.decoded.i420`。
- [ ] 调用 `vpxdec` 解码 libvpx 输出。
  - 验收：生成 `<sample>.libvpx.decoded.i420`。
- [ ] 统计 payload bits。
  - 验收：JSON 包含 `vp8uya_payload_bits` 和 `libvpx_payload_bits`。
- [ ] 统计 bits per pixel。
  - 验收：JSON 包含 `vp8uya_bits_per_pixel` 和 `libvpx_bits_per_pixel`。
- [ ] 统计 encoding fps。
  - 验收：JSON 包含 `vp8uya_fps` 和 `libvpx_fps`。
- [ ] 计算 PSNR Y/U/V/all。
  - 验收：JSON 包含 `psnr_y_db`、`psnr_u_db`、`psnr_v_db`、`psnr_all_db`。
- [ ] 计算 SSIM Y/U/V/all。
  - 验收：JSON 包含 `ssim_y`、`ssim_u`、`ssim_v`、`ssim_all`。
- [ ] 输出 NDJSON 明细。
...
- [ ] 输出复现命令。
  - 验收：Markdown 报告中失败样本包含 vp8uya、vpxenc、vpxdec 命令。
- [ ] 所有样本通过时返回 0。
  - 验收：shell `$?` 为 0。
- [ ] 任一样本失败时返回非 0。
  - 验收：shell `$?` 非 0。

## Phase 8: Makefile 与 CI 集成
...
- [ ] 增加可选 CI job。
  - 验收：CI job 名称包含 `libvpx encode threshold`。

## Phase 9: 质量追赶优化

- [ ] 记录 key/inter/skip frame 或 macroblock 计数。
  - 验收：benchmark JSON 包含 mode distribution 字段。
...
- [ ] 记录 encoder hot path allocation count。
  - 验收：benchmark JSON 包含 allocation count 或等价统计。
- [ ] 复用 frame buffers。
