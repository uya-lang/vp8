...
def parse_vp8uya_motion_distribution(stdout: str) -> dict[str, int] | None:
    field_map = {
        "encode.motion.macroblocks": "macroblock_count",
        "encode.motion.zero_mv": "zero_mv_count",
        "encode.motion.new_mv": "new_mv_count",
        "encode.motion.nonzero_mv": "nonzero_mv_count",
    }
    parsed: dict[str, int] = {}
    for line in stdout.splitlines():
        key, sep, value = line.strip().partition("=")
...
def parse_vp8uya_macroblock_mode_distribution(stdout: str) -> dict[str, int] | None:
    field_map = {
...
def parse_vp8uya_subpel_distribution(stdout: str) -> dict[str, int] | None:
    field_map = {
        "encode.subpel.macroblocks": "macroblock_count",
        "encode.subpel.half_pel_candidates": "half_pel_candidate_count",
        "encode.subpel.quarter_pel_candidates": "quarter_pel_candidate_count",
    }
    parsed: dict[str, int] = {}
    for line in stdout.splitlines():
        key, sep, value = line.strip().partition("=")
    ...
    *,
    ...
    for sample in selected:
        ...
        if subpel_distribution is not None:
            extra_fields["vp8uya_subpel_distribution"] = subpel_distribution
        ...
        results.append({
            ...
            "vp8uya_subpel_distribution_error": None if subpel_distribution is not None else "missing encode.subpel.* stdout report",
            "returncode": completed.returncode,
            "stdout": completed.stdout,
            "stderr": completed.stderr,
        })
...
def aggregate_motion_distribution(records: list[dict[str, Any]], field: str) -> dict[str, int]:
    if not records:
        raise ValueError("summary requires at least one result record")
    totals = {
        "macroblock_count": 0,
...
def aggregate_macroblock_mode_distribution(records: list[dict[str, Any]], field: str) -> dict[str, int]:
    if not records:
        raise ValueError("summary requires at least one result record")
    totals = {
        "macroblock_count": 0,
...
def markdown_int(value: Any) -> str:
    if isinstance(value, bool) or not isinstance(value, int):
        return ""
    return str(value)


def markdown_distribution_count(distribution: Any, field: str) -> str:
...
def build_markdown_report(summary: Mapping[str, Any], records: list[dict[str, Any]]) -> str:
    ...
    summary_macroblock_mode = summary.get("vp8uya_macroblock_mode_distribution")
    lines.append(
        "| "
        + " | ".join(
            [
                "summary",
                markdown_distribution_count(summary_macroblock_mode, "macroblock_count"),
                markdown_distribution_count(summary_macroblock_mode, "inter_macroblock_count"),
                markdown_distribution_count(summary_macroblock_mode, "intra_macroblock_count"),
            ]
        )
        + " |"
    )
    for record in records:
        macroblock_mode_distribution = record.get("vp8uya_macroblock_mode_distribution")
        lines.append(
            "| "
            + " | ".join(
                [
    ...
    summary_motion = summary.get("vp8uya_motion_distribution")
    lines.append(
        "| "
        + " | ".join(
            [
                "summary",
                markdown_distribution_count(summary_motion, "macroblock_count"),
                markdown_distribution_count(summary_motion, "zero_mv_count"),
                markdown_distribution_count(summary_motion, "new_mv_count"),
                markdown_distribution_count(summary_motion, "nonzero_mv_count"),
            ]
        )
        + " |"
    )
    for record in records:
        motion_distribution = record.get("vp8uya_motion_distribution")
        lines.append(
            "| "
            + " | ".join(
                [
                    markdown_cell(record.get("sample", record.get("name", ""))),
                    markdown_distribution_count(motion_distribution, "macroblock_count"),
                    markdown_distribution_count(motion_distribution, "zero_mv_count"),
                    markdown_distribution_count(motion_distribution, "new_mv_count"),
                    markdown_distribution_count(motion_distribution, "nonzero_mv_count"),
                ]
            )
            + " |"
        )
    ...
    summary_subpel = summary.get("vp8uya_subpel_distribution")
    lines.append(
        "| "
        + " | ".join(
            [
                "summary",
                markdown_distribution_count(summary_subpel, "macroblock_count"),
                markdown_distribution_count(summary_subpel, "half_pel_candidate_count"),
                markdown_distribution_count(summary_subpel, "quarter_pel_candidate_count"),
            ]
        )
        + " |"
    )
    for record in records:
        subpel_distribution = record.get("vp8uya_subpel_distribution")
        lines.append(
            "| "
            + " | ".join(
                [
                    markdown_cell(record.get("sample", record.get("name", ""))),
                    markdown_distribution_count(subpel_distribution, "macroblock_count"),
                    markdown_distribution_count(subpel_distribution, "half_pel_candidate_count"),
                    markdown_distribution_count(subpel_distribution, "quarter_pel_candidate_count"),
                ]
            )
            + " |"
        )
...
def read_vp8uya_motion_distribution(sample: Mapping[str, Any], runs_dir: Path) -> dict[str, Any]:
    try:
        path = sample_encode_metadata_path(sample, runs_dir, "vp8uya")
    except ValueError as exc:
        return {
    ...
    distribution = data.get("vp8uya_motion_distribution")
    if not isinstance(distribution, dict):
        return {
            "ok": False,
            "path": str(path),
            "motion_distribution": {},
            "error": f"encode metadata {path} must contain vp8uya_motion_distribution",
        }

    parsed: dict[str, int] = {}
    for field in ("macroblock_count", "zero_mv_count", "new_mv_count", "nonzero_mv_count"):
        value = distribution.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            return {
                "ok": False,
                "path": str(path),
                "motion_distribution": {},
                "error": f"vp8uya_motion_distribution.{field} must be a non-negative integer",
            }
        parsed[field] = value
    ...
    return {
        "ok": True,
        "path": str(path),
        "motion_distribution": parsed,
        "error": None,
    }


def read_vp8uya_macroblock_mode_distribution(sample: Mapping[str, Any], runs_dir: Path) -> dict[str, Any]:
    try:
        path = sample_encode_metadata_path(sample, runs_dir, "vp8uya")
    except ValueError as exc:
        return {
    ...
    distribution = data.get("vp8uya_macroblock_mode_distribution")
    if not isinstance(distribution, dict):
        return {
            "ok": False,
            "path": str(path),
            "macroblock_mode_distribution": {},
            "error": f"encode metadata {path} must contain vp8uya_macroblock_mode_distribution",
        }

    parsed: dict[str, int] = {}
    for field in ("macroblock_count", "inter_macroblock_count", "intra_macroblock_count"):
        value = distribution.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            return {
                "ok": False,
                "path": str(path),
                "macroblock_mode_distribution": {},
                "error": f"vp8uya_macroblock_mode_distribution.{field} must be a non-negative integer",
            }
        parsed[field] = value
    ...
    return {
        "ok": True,
        "path": str(path),
        "macroblock_mode_distribution": parsed,
        "error": None,
    }
...
def read_vp8uya_subpel_distribution(sample: Mapping[str, Any], runs_dir: Path) -> dict[str, Any]:
    ...
    distribution = data.get("vp8uya_subpel_distribution")
    if not isinstance(distribution, dict):
        return {
            "ok": False,
            "path": str(path),
            "subpel_distribution": {},
            "error": f"encode metadata {path} must contain vp8uya_subpel_distribution",
        }

    parsed: dict[str, int] = {}
    for field in ("macroblock_count", "half_pel_candidate_count", "quarter_pel_candidate_count"):
        value = distribution.get(field)
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            return {
                "ok": False,
                "path": str(path),
                "subpel_distribution": {},
                "error": f"vp8uya_subpel_distribution.{field} must be a non-negative integer",
            }
        parsed[field] = value
...
def write_results_ndjson_payload_bits(
    *,
    ...
    for sample in selected:
        ...
        vp8uya_subpel_distribution = read_vp8uya_subpel_distribution(sample, runs_dir)
        if not vp8uya_subpel_distribution["ok"]:
            failure_reasons.append(f"vp8uya subpel distribution: {vp8uya_subpel_distribution['error']}")
